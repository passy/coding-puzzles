...
main = do
  ...
  let paths = findWord mat word mempty <$> starts
...
findWord :: CMatrix -> String -> Set.Set (Int, Int) -> (Int, Int) -> Node Step
findWord matrix (w:w':ws) visited (x, y) =
  let directions = [ (x + 1, y)
      ...
      -- The Set of coordinates we've looked  at before plus the one
      ...
      nonVisitedCoords = filter (`Set.notMember` visited) coords
